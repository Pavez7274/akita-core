...
class ContextUser {
    ...
    get creation() {
        return {
            timestamp: this.__user__.createdAt,
    ...
    avatar(format, dinamyc = true) {
        return dinamyc && this.data instanceof oceanic_js_1.Member
            ? this.data.avatarURL(format, 2048)
            : this.__user__.avatarURL(format, 2048);
    }
    banner() {
        return this.__user__.banner ?? null;
    }
